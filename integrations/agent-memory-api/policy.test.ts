...
Deno.test("scopeMatches blocks personal memory unless personal visibility is requested", () => {
  assertEquals(scopeMatches(memory({ visibility: "personal" }), {
    workspace_id: "workspace-1",
    project_id: "project-1",
    channel: {},
    scope: baseScope,
...
Deno.test("scopeMatches respects project_only project filter", () => {
  assertEquals(scopeMatches(memory({ project_id: "other-project" }), {
    workspace_id: "workspace-1",
    project_id: "project-1",
    channel: {},
    scope: baseScope,
...
Deno.test("scopeMatches blocks channel memory outside the requested channel", () => {
  assertEquals(scopeMatches(memory({ visibility: "channel", channel_id: "channel-2" }), {
    workspace_id: "workspace-1",
    project_id: "project-1",
...
Deno.test("scopeMatches allows workspace memory in project recall but blocks organization memory unless requested", () => {
  assertEquals(scopeMatches(memory({ visibility: "workspace", project_id: null }), {
    workspace_id: "workspace-1",
    project_id: "project-1",
    channel: {},
    scope: baseScope,
  }), true);
  assertEquals(scopeMatches(memory({ visibility: "organization", project_id: null }), {
    workspace_id: "workspace-1",
    project_id: "project-1",
    channel: {},
    scope: baseScope,
...
Deno.test("reviewTransition marks a merged memory as merged and non-instructional", () => {
  assertEquals(reviewTransition({ action: "merge", related_memory_id: "target-memory" }), {
    memoryUpdates: {
...
Deno.test("reviewTransition marks the related memory superseded when current memory supersedes it", () => {
  assertEquals(reviewTransition({ action: "supersede", related_memory_id: "old-memory" }), {
    memoryUpdates: {},
    relatedMemoryUpdates: {
