...
import { READ_ONLY_ERROR, parseBooleanEnv, shouldBlockWriteEndpoint } from "./read-only.ts";
...
Deno.test("shouldBlockWriteEndpoint blocks all write routes in read-only mode", () => {
  ...
  assertEquals(shouldBlockWriteEndpoint("POST", "/recall/:request_id/usage", true), true);
  assertEquals(shouldBlockWriteEndpoint("PATCH", "/memories/:id/review", true), true);
});
...
Deno.test("shouldBlockWriteEndpoint leaves read routes available", () => {
  ...
  assertEquals(shouldBlockWriteEndpoint("GET", "/memories/review", true), false);
  assertEquals(shouldBlockWriteEndpoint("GET", "/recall-traces/:request_id", true), false);
