...
export function ProvenanceBadge({ value }: { value: string }) {
  return (
    <span className="inline-flex items-center rounded-full border border-info/30 bg-info/10 px-2.5 py-0.5 text-xs font-medium text-info">
      {value}
    </span>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {policy.can_use_as_instruction && (
        <span className="rounded-full border border-success/20 bg-success/10 px-2.5 py-0.5 text-xs text-success">
          instruction
        </span>
      )}
      {policy.can_use_as_evidence && (
        <span className="rounded-full border border-info/20 bg-info/10 px-2.5 py-0.5 text-xs text-info">
          evidence
        </span>
      )}
      {policy.requires_user_confirmation && (
        <span className="rounded-full border border-warning/20 bg-warning/10 px-2.5 py-0.5 text-xs text-warning">
          needs review
        </span>
      )}
      {!policy.can_use_as_instruction && !policy.can_use_as_evidence && (
        <span className="rounded-full border border-danger/20 bg-danger/10 px-2.5 py-0.5 text-xs text-danger">
          blocked
        </span>
      )}
    </div>
